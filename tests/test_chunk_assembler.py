...
def test_def_row_source_decodes_multibyte_utf8():
    ...
    encoded source. _DefRow.source() must slice on bytes and decode the
    result."""
...
async def test_assemble_python_chunks(clean_repo, python_fixture_root: Path):
    ...
    stats = await _full_pipeline(pool, repo_id, branch_id, python_fixture_root)
...
async def test_chunk_metadata_lists_external_deps(clean_repo, python_fixture_root: Path):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            WHERE c.branch_id=$1 AND d.qualified_name='main' AND c.granularity='module'
            """,
            branch_id,
        )
...
async def test_assemble_solidity_chunks(clean_repo, solidity_fixture_root: Path):
    pool, repo_id, branch_id = clean_repo
    stats = await _full_pipeline(pool, repo_id, branch_id, solidity_fixture_root)
    assert stats.n_function > 0
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            WHERE c.branch_id=$1 AND d.qualified_name='Vault.Vault.deposit'
...
async def test_chunk_idempotency(clean_repo, python_fixture_root: Path):
    pool, repo_id, branch_id = clean_repo
    first = await _full_pipeline(pool, repo_id, branch_id, python_fixture_root)
    second = await assemble_chunks(pool, repo_id, branch_id)
